...
9. **Ollamaによる繁体字・簡体字双方向変換および同義語検索拡張**:
   - **字面変換と同義語の両対応**: 中国語キーワード検索時、Ollamaを活用して文字レベルの繁簡変換（繁体字「專案」<-> 簡体字字面「专案」）および両岸（台湾・中国大陸）の技術・ビジネス慣用同義語（「專案」<->「项目」、「程式碼」<->「代码」、「記憶體」<->「内存」、「網路」<->「网络」など）を自動展開し、複合Luceneクエリ（例: `((專案 OR 專案*) OR (专案 OR 专案*) OR (项目 OR 项目*))`）を構築してシームレスな横断検索を実現。
   - **英数字クエリのゼロオーバーヘッド**: 英数字検索（例: `EFC-109`、`login`）時は即座にバイパス（0ms遅延）。
   - **メモリLRUキャッシュによる高速応答**: スレッドセーフなLRUキャッシュ（容量1,000件）により、再検索時は1ms未満で即座に展開。
   - **タイムアウト設定（デフォルト6秒）とサーキットブレーカー**: `llm.search.expansion.enabled`（デフォルト true）および `llm.search.expansion.timeout`（デフォルト 6秒）を追加。Ollamaオフライン時は自動で原語検索にフォールバックし、30秒間のサーキットブレーカーで待機遅延を防止。
10. **Markdown レンダリングにおける Windows UNC パスおよびバックスラッシュの完全保護 (Markdown Windows UNC Path & Backslash Preservation)**:
    - **根本原因の修正**: チケット詳細やコメントに Windows UNC 共有パス（例: `\\hlmt.com.tw\SysVol\hlmt.com.tw\Policies\{9CECF8CB-B752-4E7C-A1FB-90CB3B021A07}\User\Scripts\Logon` や `"\\hlmt.com.tw\..."`）を入力した際、CommonMark の記号エスケープ仕様により先頭のダブルバックスラッシュ `\\` がシングルバックスラッシュ `\` に短縮されたり、波括弧前のバックスラッシュ `\{` が消失する問題を解消しました。
    - **100% 自然なテキスト表示**: インラインコード装飾を強制せず、周囲の通常テキストと同じ自然なフォントおよびレイアウトでそのまま描画します。
    - **包括的なパス対応とコードブロック保護**: UNC 共有パス（`\\server\share`）、ローカルドライブパス（`C:\...`）、相対パス、単独のダブルバックスラッシュ（`\\`）に完全対応。インラインコード（`` `...` ``）やフェンスドコードブロック（```` ```...``` ````）は自動検出してスキップするため二重エスケープが発生せず、通常の Markdown 記号エスケープも正常に動作します。
11. **JTrac NG カプセルバッジ付きベクターロゴおよび「Lightweight Knowledge Query System」ヘッダー刷新**:
    - **フルベクター「JTrac [NG]」ブランドロゴ**: 15年前の旧式ラスタGIFを刷新し、ロイヤルブルーの `JTrac` とシアンブルーのグラデーションカプセルに白文字の `NG` を配した高精細ベクターSVGロゴを導入。Retina画面でもシャープに表示され、明暗テーマに自動適応。透過PNGおよび互換GIFも同時配備し後方互換性を100%維持。
    - **ヘッダー説明文の現代化**: 従来の「JTrac - Open Source Issue Tracking System」から「`Lightweight Knowledge Query System`」（軽量ナレッジ検索システム）へと全面刷新。`&` によるエスケープ問題を解消し、左のロゴと右の説明文が重複なくシンプルに連携。
    - **モバイル向けコンパクトロゴ＆左寄せ間隔最適化 (Mobile Compact Logo & Tight Left-Aligned Header)**: スマートフォンやモバイル端末（`<= 768px` および `<= 480px`）向けに、ロゴ高さを26px（小型スマホでは22px、最大幅90〜110px）に自動縮小。旧式の `width="20%"` や固定 `height="55px"` を撤廃し、ロゴ直後にヘッダー文を左寄せ配置することで横幅を最大限に確保し、文字の圧迫や不自然な縦折り返しを解消。
