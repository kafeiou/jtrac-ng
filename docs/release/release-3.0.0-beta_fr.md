...
8. **Modernisation de la recherche Lucene et affichage de l'historique (Sous-tokens, jokers et affichage de l'historique par défaut)** :
   ...
   - **Navigation épurée des espaces et expansion de l'historique par recherche (Clean Space Browsing & Search-Driven History Expansion)** : La navigation d'espace applique par défaut `showHistory = false` pour afficher des listes de tickets claires (chaque ticket apparaît une seule fois sous la forme `EFC-109`), éliminant l'encombrement visuel des révisions non recherchées. La vue d'historique (`showHistory = true`) est activée dynamiquement lors d'une recherche par mot-clé ou dans le formulaire avancé, et se replie automatiquement lors de l'effacement de la recherche.
   - **Ordre chronologique de l'historique (Ticket principal avant les révisions)** : Ajustement de l'ordre de tri avec historique actif pour trier par numéro de ticket décroissant et identifiant d'historique croissant (`parent.id DESC, id ASC`). Cela garantit que le ticket principal (ex: `xxx-109`) précède toujours ses commentaires de révision (ex: `xxx-109(1)`), tout en maintenant le regroupement par ticket lors du tri par d'autres colonnes.
