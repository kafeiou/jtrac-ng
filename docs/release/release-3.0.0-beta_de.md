...
8. **Modernisierung der Lucene-Volltextsuche & Historienanzeige (Sub-Tokens, Wildcard-Erweiterung & Standard-Historienansicht)**:
   ...
   - **Intelligente Wildcard-Erweiterung & Führende Platzhalter**: Erweitert Suchbegriffe automatisch zu `(term OR term*)`, aktiviert `allowLeadingWildcard = true` für `*begriff*` und unterstützt Phrasen-Toleranz (`phraseSlop = 2`).
    - **Übersichtliches Durchsuchen von Spaces und suchgesteuerte Historienerweiterung (Clean Space Browsing & Search-Driven History Expansion)**: Beim Durchsuchen von Spaces ist standardmäßig `showHistory = false` aktiv, um eine saubere Ticketliste anzuzeigen (jedes Ticket erscheint einmal als `EFC-109`). Die Verlaufsansicht (`showHistory = true`) wird dynamisch bei der Stichwortsuche oder im erweiterten Suchformular aktiviert und beim Löschen der Suche automatisch wieder eingeklappt.
    - **Chronologische Verlaufssortierung (Hauptticket vor Revisionskommentaren)**: Die Sortierung bei erweitertem Verlauf wurde auf Ticketnummer absteigend und Verlaufs-ID aufsteigend (`parent.id DESC, id ASC`) angepasst. Dadurch steht das Hauptticket (z. B. `xxx-109`) stets vor den Revisionskommentaren (z. B. `xxx-109(1)`), auch bei Sortierung nach anderen Spalten.
